...
const About = () => {
  ...
  return (
    <>
    <div className='flex flex-col px-20 h-screen w-screen'>
        <div className='text-black text-6xl font-semibold py-32'>
            What we do at<br/> tech star. <span className='text-xs font-semibold leading-none'>More than connecting tech enthusiasts</span>
        </div>
        <div className='grid grid-cols-3 gap-x-7 gap-y-20'>
            {niche.map(({ id, src, title, content }) => (
                <div
                key={id}
                className={`bg-indigo-50 flex flex-col p-5 text-center items-center  justify-center py-2 rounded-lg`}
                >
                <img src={src} alt="" className="w-10" />
                <p className="mt-4 items-center text-base font-bold">{title}</p>
                <p className="mt-4 items-center">{content}</p>
                </div>
            ))}
        </div>
    </div>
    <div className='flex max-h-screen '>
        <img src={Smiling} alt="" className="h-96" />
        <div className='flex flex-col'>
                <p>We are here for you, your growth is something we do care about.</p>
